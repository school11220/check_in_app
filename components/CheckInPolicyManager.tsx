...
export default function CheckInPolicyManager({ eventId, isAdmin = false }: { eventId: string; isAdmin?: boolean }) {
  ...
  return <section className="rounded-2xl border border-[#1F1F1F] bg-[#141414] p-5">
    <div className="mb-4 flex items-start gap-3"><Shield className="h-5 w-5 text-[#E11D2E]" /><div><h3 className="font-semibold text-white">Manual check-in permissions</h3><p className="text-sm text-[#737373]">Control whether authorized scanners can manually find and check in attendees.</p></div></div>
    <div className="space-y-3">
      ...
      <p className="flex items-center gap-2 text-xs text-[#A3A3A3]"><CheckCircle className="h-4 w-4 text-green-400" />Manual check-ins retain device and operator details in the audit trail.</p>
    </div>
  </section>;
}
